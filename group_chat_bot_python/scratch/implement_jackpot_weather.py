...
# 2. Update Tower Logic with Weather
tower_update = """
...
# Replace Casino
# Finding casino_command start and end
casino_start = content.find('@router.message(Command("casino"')
if casino_start != -1:
    # Find next command or function to find end
    casino_end = content.find('@router.callback_query(F.data.startswith("dice_double_"))', casino_start)
    if casino_end != -1:
        content = content[:casino_start] + casino_update + "\\n" + content[casino_end:]

# Replace Tower
tower_start = content.find('# --- ИГРА: БАШНЯ ФОРТУНЫ (ULTIMATE VERSION) ---')
if tower_start == -1:
    tower_start = content.find('# --- ИГРА: БАШНЯ ФОРТУНЫ ---')
    
if tower_start != -1:
    tower_end = content.find('return router', tower_start)
    if tower_end != -1:
        content = content[:tower_start] + tower_update + "\\n    " + content[tower_end:]

with open(file_path, 'w', encoding='utf-8') as f:
    f.write(content)

print("Casino Jackpot and Tower Weather implemented!")
