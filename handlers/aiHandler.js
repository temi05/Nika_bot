...
const aiTools = [
    ...
    { type: "function", function: { name: "update_user_notes", description: "Записать, обновить или пересобрать досье юзера (факты, привычки).", parameters: { type: "object", properties: { target_name: { type: "string", description: "Имя/@username" }, new_note_item: { type: "string", description: "Новый факт ИЛИ полное обновленное досье." }, replace_all: { type: "boolean", description: "Если true, текущее досье будет полностью заменено на new_note_item. Используй чтобы очищать мусор и объединять факты в текст." } }, required: ["target_name", "new_note_item"] } } },
