...
- Используй @username (только латиница!) для функций поиска по юзеру. Не пытайся мутить админов — они твои создатели/друзья.
...
async function executeToolCall(toolCall, chatId, requesterId) {
    ...
    try {
        switch (fnName) {
            ...
            case 'warn_user': {
                // Ищем юзера через БД
                const { supabase, updateUser: uu } = require('../database');
                const { data: userData } = await supabase
                    .from('users')
                    .select('user_id, first_name, warns, id')
                    .eq('chat_id', chatId)
                    .or(`username.ilike.%${args.target_name.replace('@', '')}%,first_name.ilike.%${args.target_name.replace('@', '')}%`)
                    .limit(1)
                    .maybeSingle();

                if (!userData) return `Пользователь "${args.target_name}" не найден. Попробуй указать ник точнее.`;

                // Проверка на админа (нельзя варнить админов)
                const isTargetAdmin = await isAdmin(chatId, userData.user_id);
                if (isTargetAdmin) return `Я не могу наказывать администраторов, ${userData.first_name} слишком крут для варна.`;
            ...
            case 'mute_user': {
                ...
                // Проверка на админа (нельзя мутить админов)
                const isTargetAdmin = await isAdmin(chatId, userData.user_id);
                if (isTargetAdmin) return `Ха, удачи замутить админа. Я пас. ${userData.first_name} неприкосновенен.`;
