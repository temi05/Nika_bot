...
Никогда не выдумывай факты. Если сомневаешься — пропускай.
СЖИМАЙ И ОБОБЩАЙ: Если юзер спамит одной темой (например, ноет про игру), объедини это в ОДИН короткий факт. Не создавай 5 фактов про одно событие!
...
2. Биография и Статус: Имя, возраст, город, профессия, должность, отношения.
3. Глобальные Хобби и Убеждения: "фанат аниме", "считает Валорант детской игрой", "играет на гитаре".
4. Здоровье и Техника: Аллергии, фобии, характеристики ПК, марка авто.

[ЧЕРНЫЙ СПИСОК (Мгновенно в мусорку)]:
❌ Бытовуха и Еда: "хочет шаурму", "пьет пиво", "ел пиццу", "идет спать", "встал с кровати".
❌ Временные состояния: "на работе", "заболела голова", "устал", "сейчас тильтует".
❌ Временные планы/Сроки: "через 2 дня позовут в игру", "сегодня забанили", "завтра гуляет".
❌ Отношение к мелочам: "не понравился этот мем", "скинул смешное фото".
❌ Ситуативный ролплей: действия со звездочками (*обнял*, *вздохнул*).
❌ Запросы к боту: "Ника, запомни это", "Ника, поставь статус". (Бот сам это обработает через инструменты, не сохраняй это здесь).

[ПРАВИЛА ИЗВЛЕЧЕНИЯ]
- ИДЕНТИФИКАЦИЯ: Записывай факт строго на Имя того, кто это сказал. Никаких "Он/Она".
- МАКСИМАЛЬНАЯ КРАТКОСТЬ: Выжимай саму суть. (Вместо "имеет проблемы с головой и беды с башкой" -> "имеет проблемы с головой").

[ФОРМАТ ОТВЕТА (СТРОГО JSON)]
Возвращай ТОЛЬКО валидный JSON-объект. Без маркдауна.

Пример ПУСТОГО ответа (Идеально для болтовни и нытья):
{"facts": []}
...
${historyText}`;
        ...
        for (const fact of facts) {
            ...
            if (embedding) {
                // Порог 0.85 помогает отсекать факты с похожим смыслом
