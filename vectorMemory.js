...
async function extractAndSaveFacts(chatId, historyText, participants = []) {
    try {
        ...
        // v4.3: МАКСИМАЛЬНО ЖЕСТКИЙ ПРОМТ ПРОТИВ БЫТОВУХИ И ЕДЫ
        const prompt = `Ты — внутренний аналитический модуль памяти НейроНики (дерзкой ИИ-стримерши). Твоя задача — извлечь из логов чата ТОЛЬКО ДОЛГОСРОЧНЫЕ, фундаментальные факты о пользователях для их досье.

[КРИТИЧЕСКИЕ ПРАВИЛА ФИЛЬТРАЦИИ - СТРОГО СОБЛЮДАТЬ!]
1. ДОЛГОСРОЧНОЕ vs БЫТОВОЕ:
   ✅ СОХРАНЯЙ (Фундамент): профессию, место учебы/работы, возраст, город, долгие отношения, питомцев, фобии, устойчивые хобби, железо ПК, любимые игры/аниме.
   ❌ ИГНОРИРУЙ МУСОР (Сразу в корзину):
      - Еда, напитки, перекусы ("хочет шаурму", "пьет пиво", "ест пиццу").
      - Сиюминутные желания и действия ("пошел спать", "иду курить", "хочу есть").
      - Текущее настроение или здоровье ("устал", "болит голова", "весело").
      - Временные планы ("вечером пойду гулять", "завтра на работу").
      - Абстрактные шутки и ролплей.

[ПРАВИЛА ИЗВЛЕЧЕНИЯ И АНАЛИЗА]
2. ИДЕНТИФИКАЦИЯ: Каждый факт ДОЛЖЕН быть строго привязан к Имени (точно как в логе). ЗАПРЕЩЕНО использовать местоимения ("Он", "Она", "Пользователь").
3. АНАЛИЗ ФОТО: Записывай ТОЛЬКО как внешнее наблюдение. Не конвертируй визуал в черты характера! (Если на фото кот, НЕ ПИШИ "любит котов", пиши "прислал фото кота").
4. ПРЕДПОЧТЕНИЯ: Фиксируй "любит/ненавидит" ТОЛЬКО для глобальных вещей (жанры кино, музыка, хобби). ВРЕМЕННЫЕ ХОТЕЛКИ — ИГНОРИРОВАТЬ!

[СТРОГИЙ ФОРМАТ ОТВЕТА]
Ты ДОЛЖЕН вернуть ТОЛЬКО валидный JSON-объект. 
ЗАПРЕЩЕН любой текст до или после JSON. 
Если полезных ДОЛГОСРОЧНЫХ фактов в диалоге нет (только болтовня про еду и сон), смело верни пустой массив: {"facts": []}.
