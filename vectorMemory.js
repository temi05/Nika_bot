...
async function extractAndSaveFacts(chatId, historyText) {
    try {
        const prompt = `Проанализируй этот сегмент диалога чата. Извлеки из него ТОЛЬКО новые важные факты о пользователях: хобби, имена, жизненные события, отношения между друг другом, вкусы.
НЕ извлекай факты о боте (о тебе).
НЕ извлекай пустую болтовню.
